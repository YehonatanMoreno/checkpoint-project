...
class NVD_API:
    ...
    def get_CPEs_by_keyword(self, keyword: str) -> list[str]:
        cpes_list = []
        products = requests.get(f'{NVD_API.NVD_BASE_URL}/cpes/2.0/?keywordSearch={keyword}').json()["products"]
    ...
    def get_vulnerability_by_cpe(self, cpe_name: str):
        CVEs_list = []
        vulnerabilities = requests.get(f'{NVD_API.NVD_BASE_URL}/cves/2.0?cpeName={cpe_name}').json()["vulnerabilities"]
    ...
    def extract_returned_details_for_cve(self, cve_details: dict) -> dict:
        ...
        cve_version = "cvssMetricV31" if "cvssMetricV31" in cve_metrics else "cvssMetricV2"
    ...
    def extract_exploit_github_references_urls(self, references: list[dict]) -> list[str]:
        return [reference["url"] for reference in references if "github" in reference["url"] and "Exploit" in reference["tags"]]
